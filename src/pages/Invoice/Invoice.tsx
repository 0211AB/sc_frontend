...
import { viewinTab } from '../../utils/invoice';


const InvoiceTable: React.FC<{ invoices: any }> = ({ invoices }) => {
  return (
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <div className="max-w-full overflow-x-auto">
        <table className="w-full table-auto">
          ...
          {invoices.length===0 && <div className='text-center w-full flex items-center m-3'>No Invoices Found with the filter(s)</div>}
          <tbody>
            {invoices.map((packageItem: any, key: number) => (
              <tr key={key}>
                <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                  <h5 className="font-medium text-black dark:text-white">
                    {packageItem.name}
                  </h5>
                  <p className="text-sm">Rs {packageItem.items.reduce((total: number, item: any) => total + Number(item.rate * item.qty), 0)} ( +Tax )</p>
                </td>
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <div className="flex items-center space-x-3.5">
                    <button className="hover:text-primary" onClick={() => { viewinTab(packageItem, packageItem.items) }}>
                      <svg
...
const Invoice: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        const res = await fetch(`${import.meta.env.VITE_BASE_URL}/invoice/all${queryString}`, {
          method: "GET",
          headers: {
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="All Invoice" />
      <Toaster />
      {loading === true ? <Loader height='full'></Loader> : <><div className="flex flex-col gap-9 mb-10">
      ...
      </div><InvoiceTable invoices={invoices} /></>}
