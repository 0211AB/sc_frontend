...
interface DropdownProps {
    id: string;
    setSelectedCategories: (selectedValues: string[]) => void;
    initiallySelectedCategories: string[]
}

const MultiSelectCategory: React.FC<DropdownProps> = ({ id, setSelectedCategories, initiallySelectedCategories }) => {
    ...
    useEffect(() => {
        const getAllData = async () => {
            try {
                const res = await fetch(`${import.meta.env.VITE_BASE_URL}/category/all`, {
                    method: "GET",
                    headers: {
...
const EditProduct = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const res = await fetch(`${import.meta.env.VITE_BASE_URL}/product/one/${id}`, {
                    method: "GET",
                    headers: {
                        "Content-Type": "application/json",
                        "Authorization": `Bearer :${authCtx.token}`
                ...
                if (res.status !== 200) {
                    ...
                    if (formRef.current) {
                        (formRef.current.elements.namedItem('name') as HTMLInputElement).value = res_data.name;
                        (formRef.current.elements.namedItem('company') as HTMLInputElement).value = res_data.company;
                        (formRef.current.elements.namedItem('cost') as HTMLInputElement).value = res_data.cost_price;
                        setIsChecked(res_data.cost_price_inclusive_tax);
                        (formRef.current.elements.namedItem('materials') as HTMLInputElement).value = res_data.materials.join(",");
                        (formRef.current.elements.namedItem('description') as HTMLTextAreaElement).value = res_data.description;
                        (formRef.current.elements.namedItem('gst') as HTMLInputElement).value = res_data.gst;
                        (formRef.current.elements.namedItem('hsn') as HTMLInputElement).value = res_data.hsn_code;
                        setImages(res_data.images)
                        setSelectedColors(res_data.colors)
                        setInitialCategories(res_data.category.map((c: any) => c._id))
                    }
                }
            } catch (e: any) {
                toast.error(e.message);
    ...
    const submitHandler = async (e: any) => {
        ...
        const hsn = data.get('hsn') as string;
        ...
        try {
            ...
            const res = await fetch(`${import.meta.env.VITE_BASE_URL}/product/edit/${id}`, {
                method: "PATCH",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer :${authCtx.token}`
