...
interface DropdownProps {
  ...
  initiallySelectedColorsForEdit: string[]
}

const MultiSelect: React.FC<DropdownProps> = ({ id, setSelectedColors, initiallySelectedColorsForEdit }) => {
    ...
    const loadOptions = () => {
      ...
      if (select) {
        ...
        for (let i = 0; i < select.options.length; i++) {
          ...
          if (initiallySelectedColorsForEdit.includes(select.options[i].value)) {
            newSelected.push(i);
          }
        }
  ...
  const select = (index: number, event: React.MouseEvent) => {
    ...
    if (!newOptions[index].selected) {
      ...
      setSelectedColors([...selected, index].map((option) => options[option].value))
    } else {
      const selectedIndex = selected.indexOf(index);
      if (selectedIndex !== -1) {
        newOptions[index].selected = false;
        ...
        setSelected(newSelected);
        setSelectedColors(newSelected.map((option) => options[option].value))
      }
    }
    setOptions(newOptions);
  };
                    ...
                    {selected.map((index) => (
                      ...
                      >
                        <div className="max-w-full flex justify-between items-center gap-2">
                          <div style={{ height: '20px', width: '20px', background: options[index].value, borderRadius: '5px', border: options[index].value === 'white' ? '1px solid black' : '' }}></div>
                          {options[index].text}
                        </div>
                        ...
                        >
                          ...
                          >
                            <div className="flex w-full items-center">
                              <div style={{ height: '20px', width: '20px', background: option.value, borderRadius: '5px', border: option.value === 'white' ? '1px solid black' : '' }}></div>
